...
int	exec_outfile(t_shell *cmd)
{
	...
	if (cmd->cmd_lst->append == 1)
		fd = open(cmd->cmd_lst->append_str, O_CREAT | O_WRONLY | O_APPEND,
			0644);
	else
		fd = open(cmd->cmd_lst->redir_out_str, O_CREAT | O_WRONLY | O_TRUNC,
			0644);
	if (fd < 0)
	{
...
//     waitpid(cmd->pid[i], &status, WNOHANG);
	// while loop checking for ctrl c   with flag WNOHANG
//     i--;
//     while (i > 0)
//     {
