...
/*lexer_token.c*/
int				new_token_var_words(t_token **tokens, char *string, 
					int i, int nb_token);
int				new_token_quote(t_token **tokens, char *string,
					int i, int nb_token);
t_token			*tokenization_bis(t_shell *cmd, int *i,
...
t_token			*tokenization_special_char(t_shell *cmd, int *i,
					t_token *tok_lst, int nb_token);
t_token			*token_loop(t_shell *cmd, int i, int nb_token,
					t_token *tok_lst);

/*lexer_token_utils.c*/
t_token			*tokenization(t_shell *cmd);
int				tokenization_sub(t_shell *cmd, int i,
					t_token *tok_lst, int nb_token);
void			add_stack_back_tok(t_token **tok_lst, t_token *new);
void			new_token(t_token **tokens, char *command, 
					int nb, enum e_type type);
...
int				handle_redir_in_out_sub(t_single_cmd *new,
					t_token *temp, int fd);
...
char			*check_acc_sub(char **path_arr, char **cmd,
					char *path_cmd, char *tmp);

/*single_command_utils.c*/
int				is_builtins(char *string);
int				single_command(t_shell *cmd);
void			single_command_sub(t_shell *cmd, char *path,
					t_single_cmd *temp);
void			single_command_sub_sub(t_shell *cmd, char *path,
					t_single_cmd *temp);
...
int				exec_piped_command_sub(t_shell *cmd, int pipefd[],
					int fd, int i);
...
char			**string_var_hd_sub(t_shell *c, char *str,
					char **arr_str, int start);
...
void			export_variable_bis(t_shell *cmd, char **command,
					int i, t_shell **tmp);
...
void			delete_node_cmd(t_single_cmd **head,
					t_single_cmd *node_to_delete);
